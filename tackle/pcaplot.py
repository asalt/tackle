    ...
    def __init__(
        ...
        if annotate == False:
            ...
            if pca_params is not None:
                warnings.warn(
                    """Specifying annotation in config file is depreciated,
                it can now be specified as a command line option `--annotate`""",
                    DeprecationWarning,
                )
        ...
        components = pd.DataFrame(
            data=V, columns=X.columns
        )  # should be same as col_data.index except removal of (any) experiments  with no data
        ...
        if self.metadata:
            ...
            if pca_params is not None and "color" in pca_params:
                ...
                try:
                    ...
                    warnings.warn(
                        "The label {} is not in the metadata".format(color_label)
                    )
                    n_colors = 1
        ...
        if color_label and not self.metadata_colors:
            ...
            color_label and self.metadata_colors and color_label in self.metadata_colors
        ):
            color_mapper = [
                self.metadata_colors[color_label].get(ix, "grey")
                for ix in df[color_label].values
            ]
        else:
        ...
        if self.marker_label:
            try:
                marker_mapper = [self.markers[ix] for ix in df[marker_label].cat.codes]
            except IndexError:
        ...
        for name, row in df.iterrows():  # plot first and second components
            ...
            if color_label:
                ...
                if name not in color_labels:
                    ...
                    color_handle = matplotlib.patches.Rectangle(
                        (0.0, 0.0), 0.5, 0.5, color=color  # (x,y)  # width
                    )
            ...
            if marker_label:
                ...
                if name not in marker_labels:
                    ...
                    marker_handle = plt.Line2D(
                        (0, 1), (0, 0), color="gray", marker=marker, linestyle=""
                    )
...
class PCAplot(Deconvoluter):
    ...
    COMPONENT_NAME = (
        "Principal Component"  # redefine this as appropriate for labeling on plots
    )
...
class ICAplot(Deconvoluter):
    ...
    COMPONENT_NAME = (
        "Independent Component"  # redefine this as appropriate for labeling on plots
    )
