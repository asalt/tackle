...
rc = {"font.family": "sans-serif", "text.usetex": False, "svg.fonttype": "none"}
...
def calc_kmeans(data, nclusters, seed=None, max_autoclusters=30):
    ...
    if nclusters == "auto":
        nclusters, autofig, autoax = calc_optimal_clusters(
            data, end=max_autoclusters, random_state=seed
        )
    ...
    clusters_categorical = pd.Series(
        pd.Categorical(clusters, categories=cluster_order, ordered=True),
    ...
    for c in clusters_categorical.cat.categories:
        ...
        _linkage = hierarchy.linkage(
            data.loc[ids], method="ward", optimal_ordering=True
        )
        _dend = hierarchy.dendrogram(_linkage, no_plot=True)
        _order = _dend["leaves"]
    ...
    silhouette_scores = silhouette_samples(
        data.loc[clusters_categorical.index], clusters_categorical.values
    )
    ...
    cmap_mapping = {
        val: rgb2hex(c)
        for val, c in enumerate(sb.color_palette("hls", n_colors=nclusters))
    }
    print(list(rgb2hex(x) for x in sb.color_palette("hls", nclusters)))
    print("tackle cmap", cmap_mapping)
    cluster_colors = (
        clusters.loc[clusters_categorical.index].map(cmap_mapping).to_frame("Cluster")
    )

    # print(cluster_centers.shape)
    figsize = [
        max(cluster_centers.shape[1] * 0.75, max(cluster_centers.shape[0] * 0.5, 16)),
        cluster_centers.shape[0],
    ]
    # print(figsize)
    ccenter_fig, ccenter_axs = plt.subplots(
        nrows=cluster_centers.shape[0], sharex=True, sharey=True, figsize=figsize
    )
    ...
    for ax, category in zip(ccenter_axs.flat, clusters_categorical.cat.categories):
        ...
        ax.axhline(y=0, ls="--", color="grey", alpha=0.6)
...
def clusterplot(
    ...
    if (
        dbscan or nclusters
    ):  # do not perform hierarchical clustering and KMeans (or DBSCAN)
        row_cluster = False
    ...
    if highlight_gids:
        ...
        cmap = [
            rgb2hex(x)
            for x in sb.color_palette("hls", n_colors=max(6, len(highlight_gids)))
        ]
        colors_dfs = list()
        for ix, hgid in enumerate(highlight_gids):
            ...
            colors_df = pd.Series(colors, index=data.index).to_frame(
                highlight_gid_names[ix]
            )
    ...
    if col_data is not None:
        ...
        _legend_data = (
            set(legend_include) - set(legend_exclude) if legend_include else col_data
        )
        # _legend_data = sorted(_legend_data)
        _legend_data = [
            x for x in legend_include if x in _legend_data
        ]  # preserve order
        ...
        col_colors = pd.DataFrame(columns=col_data.columns, index=col_data.index)
        ...
        for info in col_data.columns:
            ...
            if n_colors <= 10:
                ...
                colors = sb.cubehelix_palette(n_colors, start=0.1, rot=0.75)
            cmap = iter(rgb2hex(x) for x in colors)
            ...
            colors = col_data[info].map(
                mapping
            )  # cannot use np.nan as a dictionary key!
            if colors.isna().any():
                colors.loc[
                    colors.isna()
                ] = "grey"  # cannot use np.nan as a dictionary key!
    ...
    if gene_symbols:  # change index to symbols
        ...
        clustermap_symbols = [
            gid_symbol.get(str(x), _genemapper.symbol.get(str(x), str(x)))
            for x in data.index
        ]
    ...
    if standard_scale is not None:
        ...
        if z_score_by:
            data_t = pd.concat(
                [
                    sb.matrix.ClusterGrid.z_score(
                        data[col_data[col_data[z_score_by] == x].index], z_score
                    )
    ...
    if nclusters is not None:
        ...
        clusters_categorical = kmeans_result[
            "clusters_categorical"
        ]  # optimally ordered for visualization

        plot_data = data_t.loc[clusters_categorical.index]

        cmap = iter(
            rgb2hex(x) for x in sb.color_palette("hls", n_colors=max(6, nclusters))
        )
        cmap_mapping = {val: next(cmap) for val in range(nclusters)}
        cluster_colors = (
            clusters.loc[clusters_categorical.index]
            .map(cmap_mapping)
            .to_frame("Cluster")
        )
        ...
        if not gene_symbols:
            cluster_data["GeneSymbol"] = cluster_data.index.map(
                lambda x: gid_symbol.get(x, _genemapper.symbol.get(x, x))
            )
        elif gene_symbols:
        ...
        fig, ax = silhouette_plot(data.loc[valid_gids], clusters.loc[valid_gids].values)
    ...
    heatmap_height_ratio = 0.8  # this is the default (seaborn). Needs to be increased when figs are very long
    ...
    if figsize is None:
        ...
        figheight = 12
        heatmap_height_ratio = 0.8  # this is the default from Seaborn
    ...
    heatmap_width_ratio = 0.8  # default
    if gene_symbols:  # make sure there is enough room for the symbols
        heatmap_height_ratio = max(0.8, 0.8 * (0.2 * (figheight - 12)))
        # heatmap_width_ratio = 1.7
        # dendrogram_width_ratio = .06
        # print(figheight, heatmap_height_ratio)
        pass
    if figheight > 12:
        dendrogram_width_ratio = 0.16 + max(
            0, (0.1 * np.log10(figwidth)) * (figwidth - 12)
        )
    ...
    plotter = MyClusterGrid(
        plot_data,
        figsize=figsize,
        row_colors=row_colors
        if row_colors is not None and not row_colors.empty
        else None,
    ...
    if annot_mat is not None:
        ...
        g.ax_row_dendrogram.text(
            0, -0.14, _txt, transform=g.ax_row_dendrogram.transAxes, fontsize=8
        )
    ...
    for tick in g.ax_heatmap.xaxis.get_ticklabels():
        ...
        if len(tick.get_text()) > 10:
            tick.set_size(tick.get_size() * 0.75)
    if g.ax_row_colors:
        ticks = g.ax_row_colors.xaxis.get_ticklabels()
        if len(ticks) > 1:
            scale = 0.9 / (np.log1p(len(ticks)) * 0.9)
            for tick in ticks:
    ...
    if row_colors is not None and "Cluster" in row_colors.columns:
        ...
        _yticklabels = [
            "" if ix not in _positions else _positions[ix] + 1
    ...
    if col_colors is not None:
        ...
        col_label_lengths = (
            col_data.astype(str).applymap(len).max(0) + col_colors.nunique()
        )
        ...
        bboxes = [(x, bbox_y0, 1, 0.2) for x in widths]  # (x0, y0, width, height)
        ...
        for bbox, ix in zip(bboxes, col_colors_t.index):
            ...
            # ncols = min(ncols, 20)
            leg = g.ax_col_dendrogram.legend(
                handles,
                labels,
                bbox_to_anchor=bbox,
                loc="upper left",
                ...
                frameon=False,
                title=textwrap.fill(
                    col_name.replace("_", " "), 8, break_long_words=False
                ),
            )
            legends.append(leg)
    ...
    if col_colors is not None and not col_colors.empty:
        ...
        longest_label = (
            max(len(x) for x in col_colors.columns) + 6
        )  # add a little padding
        char_width = (
            430 / 1000
        )  # approx via from https://www.math.utah.edu/~beebe/fonts/afm-widths.html
