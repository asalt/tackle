...
def _render_script_template(template_name: str, **context: object) -> str:
    template_dir = Path(__file__).resolve().parent / "templates"
    # Default Jinja comments collide with Bash array lengths such as ${#files[@]}.
...
def summarize_config(conf_path: str, *, max_values: int = 12) -> ConfigSummary:
    ...
    recommended = (
        value_count_df.loc[
            (value_count_df["count"] > 1) & (value_count_df["count"] < total)
        ]
...
def render_tacklerun_skeleton(
    *,
    ...
    suggested_design = (
        summary.recommended_design_columns[0]
        if summary.recommended_design_columns
        else ""
    )
    file_format_flags = "\n".join(f"    --file-format {fmt}" for fmt in file_formats)
    only_local_flag = (
        "    --only-load-local" if only_load_local else "    # --only-load-local"
    )

    name_default = name or f'{summary.analysis_name}_$(date +%Y%m%d)'

    return _render_script_template(
        "tacklerun.sh.j2",
        generated_at=now,
        conf_path=conf_path,
        ...
        config_summary="\n".join(summary.lines),
        file_format_flags=file_format_flags,
        normalization_flag=normalization_flag,
        non_zeros=non_zeros,
        only_local_flag=only_local_flag,
    )
