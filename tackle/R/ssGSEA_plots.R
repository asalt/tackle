...
make_plots <- function(input.ds, gene.set.database, basedir = ".", geneset = "hallmark", rank_plots = TRUE) {
  ...
  heatmap.2(gsea.score,
    trace = "none", margins = c(9, 17), col = bluered(99),
    cellnote = annot,
    density.info = "none",
  ...
  text(0.08,
    y = 0.04, labels = "* p.adj < .05", adj = NULL,
    ...
    for (samp in 1:length(all.samp)) {
      ...
      if (length(grep(";u$|;d$", signat[[gs.name]], value = t)) > 0) {
        ...
        legend("bottom", legend = c(
          paste("no. down-regulated in signature:", length(grep(";d$", signat[[gs.name]]))),
          paste("no. found in data set:", length(loc.d))
        ), inset = .05, bty = "n", text.col = "darkblue")

        legend("top", legend = c(
          paste("no. up-regulated in signature:", length(grep(";u$", signat[[gs.name]]))),
        ...
        ## some info
        legend("bottom", legend = c(
          paste("no. in signature:", length(signat[[gs.name]])),
