...
#' This reduces the degrees of freedom by the number of batches.
#' see https://support.bioconductor.org/p/63007/#63115
pvalue.batch <- function (dat, mod, mod0, nbatch=0)
{
  n <- dim(dat)[2]
  m <- dim(dat)[1]
  df1 <- dim(mod)[2]
  ...
  rm(resid0)
  fstats <- ((rss0 - rss1)/(df1 - df0))/(rss1/(n - df1 - nbatch))
  p <- 1 - pf(fstats, df1 = (df1 - df0), df2 = (n - df1 - nbatch))
  return(p)
}
