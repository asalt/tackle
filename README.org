...
* Caching (optional)
  The data loading and normalization stages can be cached to speed up reruns with
  small variations. This cache is disabled by default and can be enabled via
  environment variables. Cache files are stored alongside the results directory
  (e.g., if your results dir is =./results=, the cache dir is =./.tackle_cache=).

  Environment variables:
  - =TACKLE_CACHE=: Set to =1= to enable caching. Default: disabled.
  - =TACKLE_CACHE_DIR=: Override cache directory location.
  - =TACKLE_CACHE_MAX_ENTRIES=: Maximum number of cache entries. Default: 5.
  - =TACKLE_CACHE_MAX_BYTES=: Maximum total cache size in bytes. Default: 2147483648 (2 GB).
  - =TACKLE_CACHE_BUST=: Optional string to force a cache miss when changed.

  Example:
  #+BEGIN_SRC sh
  TACKLE_CACHE=1 TACKLE_CACHE_MAX_ENTRIES=5 TACKLE_CACHE_MAX_BYTES=2147483648 \
    correlationplot ./example.config cluster
  #+END_SRC
* Limma formulas and gene covariates

When using limma via the `--formula` option, you can reference a gene's
expression on either side of the formula.
...
- Symbols still work: e.g., `HER2 ~ 0 + condition` preserves `HER2` on the LHS,
  injects that gene's values into the sample metadata, and fits the model using
  the full formula exactly as written. This does not subset the expression
  matrix or reinterpret `HER2` as an RHS coefficient target.
- Gene references on the RHS still behave as covariates: `~ HER2 + condition`
  injects HER2 expression on the RHS and returns the usual coefficient/contrast
  output for the fitted design terms.
...
- `GID_851751 ~ 0 + group`  # preserve the response term and fit the full formula
- `GID_851751 + GID_1234 ~ 0 + group`  # multiple LHS response terms are allowed
...
- Options:
  ...
  - `--no-style` to disable minimal formatting (freeze header row, auto-filter, column widths)

This requires an Excel writer engine (either `openpyxl` or `xlsxwriter`).

Recommended installation:
#+BEGIN_SRC sh
pip install -e '.[excel]'        # editable checkout
# or (if installing from a package)
pip install 'tackle[excel]'
#+END_SRC

Optional: install `.[excel-fast]` to also pull in `pyarrow` for faster TSV reads.

* Slide deck table summaries

To create slide-ready summaries from volcano TSV outputs, you can generate a bundle of
PNG/PDF table images (and optionally a PowerPoint deck):

- `tackle make-deck --base-dir <analysis_outdir>`  # writes to `<analysis_outdir>/report/deck/`
- Options:
  - `--filter SUBSTR` (repeatable) to include only matching volcano TSVs
  - `--topn N` and `--direction both|up|down` to control top-hit selection
  - `--p-cutoff 0.05` (set to 0 to disable) and `--fc-cutoff` to filter hits
  - `--no-pptx` to skip deck generation (still writes table images)

To enable PowerPoint output, install:
#+BEGIN_SRC sh
pip install -e '.[slides]'
# or
pip install 'tackle[slides]'
#+END_SRC
