...
- `cluster2`: clustered feature heatmaps. `--z-score row` is the default and scales each gene across samples; `--z-score column` (short alias `col`) scales each sample across genes. Numeric z-score compatibility aliases follow seaborn: `0=row`, `1=column`. Optional `--standard-scale row|column` performs sum scaling and remains off by default; its legacy aliases preserve tackle's existing behavior: `1=row`, `0=column`.
